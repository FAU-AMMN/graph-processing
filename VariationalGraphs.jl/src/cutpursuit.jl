...
function [E_red, w_red] = ComputeReducedGraph(edges::Array{T, 2}, 1}, 
                                              w::Array{T, 1}, 
                                              cutedge::Array{Bool, 1}, 
                                              bins::Array{Int64, 1}) where T
    for i = 1:length(ccmps)
        for j = 1:length(ccmps)
            
    
    
    w_red = accumarray(ic,w(cutedge));
end
    ...
    if cutType == "aniso"
        ...
        for i = 1:d
            ...
            while iter <= 1 #&& ~isempty(find(cutedge==0,1))
                ...
                g_flow, c_matrix, gradJS, Sc = generate_flowgraph(1, f0[((i - 1) * N):(i * N)], 
                                                                  w0, f[((i - 1) * N):(i * N)], 
                                                                  g, cutAlpha, "aniso")
                ...
                # Convergence?
                if conv
                    disp("Algorithm converged, minimal cut found.")
                    break
                end
